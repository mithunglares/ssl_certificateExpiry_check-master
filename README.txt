...
3. Enable suitable connectivity from the host Server where the utility is being deployed(AG) to the Domain URL.

Configuration / SOP:
1. Configuration File: Create a JSON configuration file (config.json) with the following structure:
...
2. Domains File: Create a CSV file (domains.csv) with each line containing a domain name and URL separated by a comma. For example:
example.com, https://example.com
example.net, https://example.net

3. chmod a+x <path to the ssl_expiry_script_v1.1>/ssl_expiry_script_v1.1
